...
function TimelineItem({ entry, index }: { entry: ExperienceEntry; index: number }) {
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="absolute left-[11px] top-[6px] bottom-0 w-px bg-border last:hidden"
      />

      {/* Timeline dot */}
      <div
        aria-hidden
        className="absolute left-0 top-[6px] flex h-[23px] w-[23px] items-center justify-center rounded-full border-2 border-[var(--brand)] bg-background"
      >
        <Briefcase size={11} className="text-[var(--brand)]" />
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm font-medium text-[var(--brand)]">{entry.company}</p>
          </div>
          <span className="text-sm text-[var(--foreground-secondary)]">{entry.period}</span>
        </div>
